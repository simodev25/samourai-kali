...
<purpose>
Invoke @tdd-orchestrator as a red team orchestrator to implement a task or phase by following strictly the cycle hypothesize → prove → document. Guarantees that hypotheses and validation checks are defined BEFORE final conclusions.
...
2. Read `.samourai/ai/agent/project-profile.md` when present and pass it to @tdd-orchestrator
...
4. For each task:
   ...
   c. @tdd-orchestrator executes cycle HYPOTHESIZE → PROVE → DOCUMENT
